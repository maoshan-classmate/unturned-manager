...
项目中的文档路径一般放在`claudedocs\` `.claude`\ `docs\`，具体的文档存放规则可以查看 `@.claude/rules/document-organization.md`

如果文档里涉及到设计、开发、任务进度等有时效期的内容

1. 查看git log 和 gif diff，根据这些git内容来判断这些文件是否已经过时。如果无法确定，需要询问用户，由用户裁定
2. 如果有证据确定该些文件已过时，该文件应被清理
