...
### 面板内编辑 LDM 配置 + 应用变更（2026-08-16 Phase 5 设计稿落地）

```
# ① 面板「Mod 框架」页 → 「框架配置」Tab / 「权限组」Tab
#    改字段（结构化编辑器） → 点「保存配置」
#    ——「插件配置」Tab 本期不做（每插件 schema 不同，留独立 Phase）

# ② 文件已落盘（运行时允许写入，不阻断 ServerManager 状态）
#    「保存配置」按钮旁常驻提示「需重启服务器才能生效」

# ③ 用户主动点 LdmPage 顶部全局「应用变更」按钮 → 面板调 POST /api/servers/:id/ldm/apply
#    → ServerManager.applyChangesCore（与 mod_apply 共用，§5.6）
#    → Save + Shutdown 10 + forceKill + spawn（ADR-0004 §重启流水线）
#    → postStartHook: PTY 写 /p reload 重载权限（Design §7.3）
#    → WS ldm_apply_progress 推进度 → 配置生效
```

> **关键边界（用户 2026-08-15 / 2026-08-16 拍板）**：写配置**运行时允许**（不强制 STOPPED），**生效需重启**——前端文案提醒 + 「应用变更」按钮（**LdmPage 顶部全局**——§10 决策）单独触发，写配置时不自动重启。

> **编辑器范围**：本期只做 3 个结构化编辑器（Rocket.config.xml / Rocket.Unturned.config.xml / Permissions.config.xml）。Monaco XML 原文编辑器**本期不做**，推到「权限组编辑器 V2」独立 Phase（设计 §10 决策）。
