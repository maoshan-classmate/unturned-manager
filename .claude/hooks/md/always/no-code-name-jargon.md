...
<!-- INJECT -->
1.所有输出内容——终端输出、文档输出、工作流输出、测试输出、任务清单输出——严禁「代码名+中文」或「行内术语」堆砌，人类开发者难以理解。只有在做代码实现设计时，才允许使用「代码名+中文」或「行内术语」，这是给开发者实施开发用的。
反例（不要这样）：让 installU3DS/updateU3DS/reinstall/checkUpdate 全部异步
正例（应该这样）：让 U3DS 相关操作和下载创意工坊模组的操作全部异步
2.在编写代码注释时，永远只保留最新版本，不允许出现历史叙述（历史叙述应该维护在文档而不是代码注释上）。如果有看到代码注释上有历史叙述的，即时清除，保留代码注释整洁性。
<!-- /INJECT -->
