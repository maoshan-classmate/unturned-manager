/**
 * 系统指标采集与查询——Dashboard 资源图（PR-2a / P3A）支撑。
 *
 * 设计边界（P3 设计稿 §2.2 拍板）：
 ...
 * - **容器/宿主总资源**：选 systeminformation 路线（D2 选项 B），不绑定具体进程。
 * - **网络指标暂不暴露**（D3 拍板前置）：容器内 + 多实例无法精确拆分，留后续 PR。
 *
 * 采样策略：
 * - 后端启动时 `start()` 启动定时采样（默认 5s 一次，D3 默认值）
