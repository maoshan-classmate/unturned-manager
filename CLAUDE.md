...
## Claude Code开发、输出内容铁律

1. 所有输出内容，包括但不限于终端输出，文档输出，架构设计输出，工作流输出、测试输出、任务清单输出，严令禁止代码名+中文或者行内术语的描述方式，人类开发者难以理解。
   ```
   错误示例：
   ...
   ```
2. 做功能修复/开发时，做最小验证，是否每个小功能都需要做测试等复杂繁琐的步骤，没有这些步骤，功能是否能正常运行。如果能，就去掉这个步骤。如果无法判定是否属于小功能，询问用户。
   
   ```
   场景1：修改dialog下拉框无法选中的问题
   错误示例：修改完代码之后，又写代码跑单元测试，集成测试，e2e全量测试，冒烟测试后完成。
   正确示例: 修改完代码之后，检查代码编译，确认无误后完成。
   场景2：解决前端与后端交互的简单传值问题
   错误示例：修改完代码之后，又写代码跑单元测试，集成测试，e2e全量测试，冒烟测试后完成。
   正确示例: 修改完代码之后，检查代码编译，确认无误后完成。
   场景3：排查并解决控制台页面无任何输出的问题
   错误示例：修改完代码之后，检查代码编译，确认无误后完成。
   正确示例: 修改完代码之后，又写代码跑单元测试，集成测试，e2e全量测试，冒烟测试后完成。
   ```
---
