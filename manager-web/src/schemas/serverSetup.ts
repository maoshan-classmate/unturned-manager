...
/**
 * 启动命令配置（startCommand，ADR-0004 Phase 4）
 * 区分 launchCommandsSchema（Commands.dat 多行文本）：
 *   - launchCommandsSchema：游戏内指令列表（Name/Port/Owner/...）
 *   - startCommandSchema：PTY bash 单行启动命令
 *
 * startCommand 走 PATCH /servers/:id 持久化；max 2048 与后端 zod schema 对齐。
 */
...
export type StartCommandForm = z.infer<typeof startCommandSchema>;
