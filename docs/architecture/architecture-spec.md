...
| **共享卷文件系统**                                                                    | 直接读写（`fs`）                              | `Servers/<ServerID>/` 目录树、`Workshop/Steam/content/304930/` 已装 Mod、`Workshop/staging/` 下载暂存、`Logs/*.log` 日志                                                                                                   |
...
| `Workshop/Steam/content/304930/`（已装 Mod） | apply 流水线移动 staging 内容  | **是**         | 写入运行中服务端直接读取的位置有覆盖风险；进程停后方可移动      |
