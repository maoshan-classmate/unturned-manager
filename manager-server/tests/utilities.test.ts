...
describe('SteamCmdManager', () => {
  it('updateU3DS: 有活跃实例时拒绝', async () => {
    ...
    const mgr = new SteamCmdManager(
      proc, makeMockBroadcaster(), '/usr/bin/steamcmd',
      () => ['S1' as ServerId],
    );
  ...
  it('updateU3DS: 全部 STOPPED 但 SteamCMD 不存在 → 拒绝', async () => {
    ...
    const mgr = new SteamCmdManager(
      proc, makeMockBroadcaster(), '/nope/steamcmd',
      () => [],
    );
...
describe('FilesService', () => {
  ...
  it('writeFile + readFile: 文本往返不破', async () => {
    await svc.writeFile('FilesServer' as ServerId, 'Commands.dat', new TextEncoder().encode('Name X\n'));
  ...
  it('敏感字段 GSLT/Password 脱敏', async () => {
    await svc.writeFile('FilesServer' as ServerId, 'openmod.yaml',
