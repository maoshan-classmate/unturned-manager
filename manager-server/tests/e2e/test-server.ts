...
fs.rmSync(path.join(resolveInstallDir(), "Servers"), {
  recursive: true,
  force: true,
});

const existing = db.prepare("SELECT id FROM users LIMIT 1").get() as
  unknown | undefined;
if (!existing) {
...
const { createModBrowseRouter } =
  await import("../../src/routes/mod-browse.js");
