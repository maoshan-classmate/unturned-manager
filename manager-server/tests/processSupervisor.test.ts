...
import { ProcessSupervisor, type TerminateTimings } from '../src/modules/process/ProcessSupervisor.js';
...
const FAST: TerminateTimings = { sigint: 10, sigterm: 10, sigkill: 10, taskkill: 10 };
...
describe('ProcessSupervisor — T6 进程生命周期（GSM 对齐）', () => {
  it('spawn: 返回 PID + 进程组登记（isRunning true）', async () => {
    ...
    const pid = await sup.spawn('S1' as ServerId, process.execPath, ['-e', LONG_RUNNING_SCRIPT]);
  ...
  it('gracefulShutdown: SIGINT/SIGTERM 被忽略 → 三段走到 SIGKILL 强制退出', async () => {
    ...
    await sup.spawn('S2' as ServerId, process.execPath, ['-e', IGNORE_SIGNAL_SCRIPT]);
  ...
  it('gracefulShutdown: 正常响应 SIGINT → 第一段即退出（无需 SIGKILL）', async () => {
    const sup = new ProcessSupervisor(FAST);
    await sup.spawn('S3' as ServerId, process.execPath, ['-e', LONG_RUNNING_SCRIPT]);
  ...
  it('forceKill: 跳过优雅段直接 SIGKILL', async () => {
    const sup = new ProcessSupervisor(FAST);
    await sup.spawn('S4' as ServerId, process.execPath, ['-e', IGNORE_SIGNAL_SCRIPT]);
  ...
  it('waitForExit: 进程自行退出 → resolve', async () => {
    const sup = new ProcessSupervisor(FAST);
    await sup.spawn('S5' as ServerId, process.execPath, ['-e', SHORT_LIVED_SCRIPT]);
  ...
  it('waitForExit: 超时 → reject', async () => {
    const sup = new ProcessSupervisor(FAST);
    await sup.spawn('S6' as ServerId, process.execPath, ['-e', LONG_RUNNING_SCRIPT]);
  ...
  it('destroy: 杀死所有子进程', async () => {
    const sup = new ProcessSupervisor(FAST);
    await sup.spawn('S7' as ServerId, process.execPath, ['-e', LONG_RUNNING_SCRIPT]);
    await sup.spawn('S8' as ServerId, process.execPath, ['-e', LONG_RUNNING_SCRIPT]);
    await sup.destroy();
    await sleep(100);
  ...
  it('spawn 重复 serverId → 拒绝', async () => {
    const sup = new ProcessSupervisor(FAST);
    await sup.spawn('S9' as ServerId, process.execPath, ['-e', LONG_RUNNING_SCRIPT]);
    await expect(
      sup.spawn('S9' as ServerId, process.execPath, ['-e', LONG_RUNNING_SCRIPT]),
