...
export function createSteamCmdRouter(steamCmdManager: ISteamCmdManager): Router {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        throw new AppError('steamcmd-check-failed', err instanceof Error ? err.message : '检查更新失败', 500);
      }
    }),
  );
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      const { installDir } = req.body as { installDir: string };
      try {
        await steamCmdManager.installU3DS(installDir);
        res.status(202).json({ data: { message: 'U3DS 安装已启动，进度由 WS steamcmd_progress 推送' } });
      } catch (err) {
        if (err instanceof AppError) throw err;
        throw new AppError('u3ds-install-failed', err instanceof Error ? err.message : 'U3DS 安装失败', 500);
      }
    }),
  );
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        res.status(202).json({ data: { message: 'SteamCMD 重装已启动，进度由 WS steamcmd_progress 推送' } });
      } catch (err) {
        if (err instanceof AppError) throw err;
        throw new AppError('steamcmd-reinstall-failed', err instanceof Error ? err.message : '重装失败', 500);
      }
    }),
  );
