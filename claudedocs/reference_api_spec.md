...
> **配套**：`reference_config_files.md`（配置文件字段）+ `reference_console_commands.md`（PTY 命令）
...
| 方法 | 路径 | 后端 handler | Zod | 前端消费点 | 状态 |
|---|---|---|---|---|---|
| GET | `/` | `ServerManager.listServers` | — | `useServer` 挂载拉一次 + 手动 refresh | ✅ |
| POST | `/` | `ServerManager.createServer` | ❌ 无 | `useServer.addServer`（ServerSetup 创建实例） | ⚠️ 脏数据直入库 |
| PATCH | `/:id` | `ServerManager.configureServer` | ❌ 无 | — | ⚠️ 僵尸端点 |
...
| POST | `/:id/restart` | `ServerManager.restart` | — | `useServerActions.restart` | ✅ |
| POST | `/:id/execute` | PTY 终端 owner-trust | — | `useConsole.sendCommand` | ✅ |
...
### 2.6 Workshop `/api/workshop`

| 方法 | 路径 | 后端 handler | Zod | 前端消费点 | 状态 |
|---|---|---|---|---|---|
| GET | `/mods/:fileId` | `WorkshopMetadataService.getModDetails` | — | `ModsPage` 添加 Mod 拉元数据 | ✅（IPublishedFileService/GetDetails，需 WebAPI Key） |

### 2.7 SteamCMD `/api/steamcmd`

| 方法 | 路径 | 后端 handler | Zod | 前端消费点 | 状态 |
|---|---|---|---|---|---|
| GET | `/status` | `SteamCmdManager.getStatus` | — | `ServerSetupPage` SteamCMD 状态 | ⚠️ 本地路径检查，Linux 路径下恒 false |
| POST | `/update` | `SteamCmdManager.updateU3DS` | ❌ 仅 typeof | `ServerSetupPage`「更新 U3DS」 | ❌ 不 spawn 但返回 202 |
...
| **ServerSetup** | 创建实例 | `POST /servers` | ⚠️ 无校验；前端纯本地效果，不调用 |
...
**当前 WS token**：前端 `WebSocketContext.tsx:28` 与 `useConsole.ts:42` 用 accessToken；后端 `gateway.ts:25` 用 `validateAccessToken` 校验。
...
| 阶段 | 目标 | 关键项 |
|---|---|---|
| 0 | 主干修复（可运行门禁） | A2S API 修正、Commands.dat 契约、WS 订阅 + LogStreamer 接线、AppError/asyncHandler/Zod 接入 |
...
- `GET /api/audit-logs` — Settings 系统日志（`index.ts:104` 挂载）
